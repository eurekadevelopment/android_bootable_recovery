...
int main(int argc, char** argv) {
  ...
  printf("Starting TWRP %s-%s on %s (pid %d)\n", TW_VERSION_STR, TW_GIT_REVISION,
         ctime(&StartupTime), getpid());
...
// This way we don't need to remount it later, just for overriding properties
      ...
      std::vector<std::string> additional_build_prop_list =
          TWFunc::Split_String(TW_SYSTEM_BUILD_PROP_ADDITIONAL_PATHS, ";");
      build_prop_list.insert(build_prop_list.end(), additional_build_prop_list.begin(),
                             additional_build_prop_list.end());
#endif
      while (getline(override_props, current_prop, ';')) {
        ...
        for (auto&& prop_file : build_prop_list) {
          string sys_val =
              TWFunc::System_Property_Get(other_prop, SarPartitionManager, "/s", prop_file);
          if (!sys_val.empty()) {
    ...
    for (index = 1; index < argc; index++) {
      ...
      if (*argptr == 'u') {
        ...
        if (*ptr) {
          ...
          if (!OpenRecoveryScript::Insert_ORS_Command(ORSCommand)) break;
        } else
        ...
        // Other 'w' items are wipe_ab and wipe_package_size which are related to bricking the
        // device remotely. We will not bother to suppor these as having TWRP probably makes
        // "bricking" the device in this manner useless
      } else if (*argptr == 'n') {
  ...
  if (Boot == NULL || Boot->Current_File_System != "emmc")
    ...
    string injectcmd =
        "injecttwrp --backup /tmp/backup_recovery_ramdisk.img bd=" + Boot->Actual_Block_Device;
  ...
  if (cacheDir == DATA_LOGS_DIR) cacheDir = "/data/cache";
  ...
  if (DataManager::GetIntValue("tw_mtp_enabled") && !strcmp(mtp_crash_check, "0") &&
      !crash_counter &&
